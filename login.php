...
<head>
    ...
    <style>
        ...
        .btn-login { 
            width: 100%; padding: 14px; background-color: #007bff; border: none; 
            color: white; border-radius: 8px; cursor: pointer; font-size: 16px; 
            font-weight: bold; margin-top: 10px; text-transform: uppercase; 
            transition: all 0.4s ease; box-shadow: 0 4px 15px rgba(0, 123, 255, 0.3);
        }
        ...
        .btn-login:disabled {
            background-color: #555555 !important; color: #cccccc; cursor: not-allowed;
        }

        .alert { padding: 12px; border-radius: 8px; font-size: 13px; text-align: center; margin-bottom: 20px; line-height: 1.5; }
        .error { background-color: #fce4e4; color: #963b3b; border: 1px solid #f9cccc; }
        .success { background-color: #d4edda; color: #155724; border: 1px solid #c3e6cb; }
        .warning-session { background-color: #fff3cd; color: #856404; border: 1px solid #ffeeba; display: flex; align-items: center; justify-content: center; gap: 8px; }
        
        .footer-links { text-align: center; margin-top: 25px; font-size: 14px; }
        .reg-link { color: #2ecc71 !important; text-decoration: none; font-weight: bold; transition: 0.3s; }
        ...
        .toggle-password { position: absolute; right: 15px; top: 50%; transform: translateY(-50%); cursor: pointer; color: #666; font-size: 18px; z-index: 10;}
    </style>
</head>
<body>
    ...
    // 1. Fungsi Toggle Password (Hanya jalan saat diklik)
    ...
    // 2. Fungsi Bersihkan URL (Langsung jalan otomatis saat halaman dibuka/refresh)
    if (window.history.replaceState) {
        const url = new URL(window.location);
        if (url.searchParams.has('pesan')) { // Cek jika ada parameter pesan
